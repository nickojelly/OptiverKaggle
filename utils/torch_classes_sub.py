...
class Stock():
    def __init__(self, stock_id:int,hidden_size=64,num_layers=2) -> None:
        ...
        self.hidden_all = torch.zeros(30,hidden_size)
        self.hidden_out = torch.zeros(49,hidden_size)
...
class TradingData():
    ...
    def process_stock_data(self, stock_id, day, stock_daily_data):
        self.stocksDict[stock_id].data_daily[day] = torch.stack([torch.tensor(x) for x in stock_daily_data['stats'].to_numpy()]).to('cpu')
    ...
    def generate_batches(self, validation_split=0.20):
        ...
        for i in tqdm(train_range):
            ...
            train_data = torch.stack([self.stocksDict[x].data_daily[i] for x in self.daysDict[i]]).to('cpu')
            train_classes = torch.stack([self.stocksDict[x].target_daily[i] for x in self.daysDict[i]]).to('cpu')
            train_wap_ohe = torch.stack([self.stocksDict[x].wap_daily_ohe[i] for x in self.daysDict[i]]).to('cpu')
            train_target_ohe = torch.stack([self.stocksDict[x].target_daily_ohe[i] for x in self.daysDict[i]]).to('cpu')


            bid_price_daily = torch.stack([self.stocksDict[x].bid_price_daily[i] for x in self.daysDict[i]]).to('cpu')
            ask_price_daily = torch.stack([self.stocksDict[x].ask_price_daily[i] for x in self.daysDict[i]]).to('cpu')

            wap_price_daily = torch.stack([self.stocksDict[x].wap_price_daily[i] for x in self.daysDict[i]]).to('cpu')
        ...
        for i in tqdm(val_range):
            ...
            ask_price_daily = torch.stack([self.stocksDict[x].ask_price_daily[i] for x in self.daysDict[i]]).to('cpu')
            wap_price_daily = torch.stack([self.stocksDict[x].wap_price_daily[i] for x in self.daysDict[i]]).to('cpu')
            actual_wap = torch.stack([self.stocksDict[x].actual_wap[i] for x in self.daysDict[i]]).to('cpu')
    ...
    def reset_hidden(self, hidden_size=32,num_layers=2, device='cuda:0'): 
        ...
        for stock in self.stocksDict.values():
            ...
            stock.hidden_out = torch.zeros(49,hidden_size).to('cpu')
    ...
    def extract_data(self, i):
        new_x = self.train_batches[i]
        Y = self.train_class_batches[i]
        Y_price_ask = self.train_ask_price_daily[i]
        Y_price_bid = self.train_bid_price_daily[i]
        Y_price_wap = self.train_wap_price_daily[i]
        Y_ohe_wap = self.train_wap_ohe_batches[i]
        Y_ohe_target = self.train_target_ohe_batches[i]

        # Add your code here

        return new_x, Y, Y_price_ask, Y_price_bid, Y_price_wap, Y_ohe_wap, Y_ohe_target
    
    def extract_val_data(self, i):
        new_x = self.val_batches[i]
        Y = self.val_class_batches[i]
        Y_price_ask = self.val_ask_price_daily[i]
        Y_price_bid = self.val_bid_price_daily[i]
        Y_price_wap = self.val_wap_price_daily[i]
        Y_ohe_wap = self.val_wap_ohe_batches[i]
        Y_ohe_target = self.val_target_ohe_batches[i]
...
class GRUNetV4(nn.Module):
    def __init__(self, input_size, hidden_size,hidden=None,output='raw', dropout=0.5, fc0_size=256,fc1_size=64,num_layers=1,target_size=7):
        ...
        self.fc_reg0 = nn.Linear(hidden_size,128)
    ...
    def forward(self, x,h=None, test=False):
        ...
        # x_reg = torch.cat([x_h,x_ohe],dim=2)
        x_reg = x_h
...
class GRUNetV4_sub(nn.Module):
    def __init__(self, input_size, hidden_size,hidden=None,output='raw', dropout=0.5, fc0_size=256,fc1_size=64,num_layers=1,target_size=7):
        super(GRUNetV4_sub, self).__init__()
        self.gru = nn.GRU(hidden_size,hidden_size,num_layers=num_layers, dropout=0.3,batch_first=True)
        self.relu = nn.ReLU()
        self.batch_norm = nn.BatchNorm1d(input_size)
        self.layer_norm = nn.LayerNorm(hidden_size)
