...
def train_model(trading_df:torch_classes.TradingData, model:torch_classes.GRUNet, config:dict, optimizer, criterion):
    ...
    for epoch in trange(epochs):
        ...
        for i in range(0,384):
            ...
            output,hidden,_,x_h = model(new_x,hidden_in)
...
def validate_model(trading_df:torch_classes.TradingData,model:torch_classes.GRUNet,criterion,epoch,):
    ...
    for i in range(0,len_val-1):
        ...
        output,hidden,relu,x_h = model(new_x,hidden_in)
        ...
        #One hot wins
        # label = torch.zeros_like(y).scatter_(1, torch.argmax(y, dim=1).unsqueeze(1), 1.)
        # pred_label = torch.zeros_like(output).scatter_(1, torch.argmax(output, dim=1).unsqueeze(1), 1.)
        # correct_tensor = label*pred_label

        correct_l = predicted == actual

        # print(f"{len(correct_l)=},{predicted}")
    ...
    print(f"Accuracy = {log_dict.correct.mean()}")
    wandb.log({"Accuracy":log_dict.correct.mean()})
